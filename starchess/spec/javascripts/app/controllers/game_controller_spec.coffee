...
describe 'strollz', ->
  ...
  describe 'StarchessGameCtrl', ->
    ...
    it 'has choose mode setup correctly', ->
      expect(@scope.game_data.game.game_status).toEqual "This is the board setup phase"
    ...
    it 'does stuff on click in choose mode', ->
      ...
      expect(@gameService.game_data.game.selected).toBeNull()
      @scope2.do_click(6,2)
      expect(@gameService.game_data.game.selected).toEqual(4)

    it 'also handles clicks in play mode', ->
      @gameService.game_data.game = @game
      @gameService.game_data.game.mode = 'play_mode'
      @gameService.game_data.game.board_state = JSON.parse('{"white":{"4":"bishop","8":"queen","17":"rook","30":"king","31":"pawn"},"black":{"5":"queen","9":"pawn","10":"bishop","18":"pawn","20":"rook","33":"pawn","34":"king"}}')
      @gameService.game_data.game.boardState = @gameService.game_data.game.board_state
      @gameService.game_data.game.turn = 'black'
      @gameService.game_data.game.extra_state.current_user_player = 'white' # other player
      @gameService.game_data.game.available_moves = {5: [6]}
      @gameService.game_data.game.selected = null
      @scope2.do_click(5,2)
      expect(@gameService.game_data.game.selected).toBeNull()

      @gameService.game_data.game.extra_state.current_user_player = 'black'

      @scope2.do_click(5,2) # select queen
      expect(@gameService.game_data.game.selected).toEqual(5) # square highlighted
      @scope2.do_click(5,2) # unselected
      expect(@gameService.game_data.game.selected).toBeNull() # square not highlighted
      @scope2.do_click(5,2)
      expect(@gameService.game_data.game.selected).toEqual(5) # square highlighted
      ...
      @gameService.game_data.game.put = () ->
        apiSpy

      @scope2.do_click(4,2) # click forward one - our one available move as set above
      expect(apiSpyThen.called).toBe(true)
      expect(@gameService.game_data.game.selected).toBeNull()
      new_state = JSON.parse(@gameService.game_data.game.board_state)
      expect(new_state['black']['6']).toEqual('queen')
      expect(@gameService.game_data.game.selected_move).toEqual('[5,6]')
